...
function showComponentEnvKVs(){
    envpairs = componentEnvData.envs;

    $("#component-envs").empty();
    _.each(envpairs,function(item,index){
        var row = '<tr data-index="'+index+'"><td>'
                    +'<input type="text" class="form-control col-md-5 c-env-key" value="'+item[0]+'" required>'
                    + '</td><td>'
                    +'<input type="text" class="form-control col-md-5 c-env-value" value="'+item[1]+'" required>'
                    + '</td><td>'
