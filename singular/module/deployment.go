...
func DeployInfraStacks(d *objects.Deployment, db bool, stdout io.Writer, timestamp bool) error {
	...
	if db == true {
		objects.WriteLog(d, "Open database connection and migrate the table struct", stdout, timestamp)
	...
	if d.Tools.SSH.Private != "" {
		...
		objects.WriteLog(d, fmt.Sprintf("Generate the SSH keys file [%s] for deploy cloud native stack", d.Tools.SSH.Private),
			stdout, timestamp)
	}
	...
	if d.Service.Provider != "" {
		...
		case "digitalocean":
			objects.WriteLog(d, "Create DigitalOcean droplets", stdout, timestamp)

			objects.WriteLog(d, "Init DigitalOcean client with token", stdout, timestamp)
			...
			objects.WriteLog(d, fmt.Sprintf("Droplets is [%v]", tags), stdout, timestamp)
			...
			for ip, id := range do.Droplets {
				...
				objects.WriteLog(d, fmt.Sprintf("Droplet [%d] ip is [%s]", id, ip), stdout, timestamp)
	...
	if len(d.Nodes) > 0 {
		...
		for _, node := range d.Nodes {
			...
			objects.WriteLog(d, fmt.Sprintf("[COUT] NODE_%d = %s", i, node.IP), stdout, timestamp)
		}

		// Initialization node environment
		for _, node := range d.Nodes {
			objects.WriteLog(d, fmt.Sprintf("Initializate the %s node environment", node.IP), stdout, timestamp)
		...
		objects.WriteLog(d, fmt.Sprintf("Generate cluster root CA files."), stdout, timestamp)
		if roots, err := tools.GenerateCARootFiles(d.Config); err != nil {
			return err
		} else {
			for key, value := range roots {
				d.Output(key, value)
				objects.WriteLog(d, fmt.Sprintf("[COUT] %s = %s", key, value), stdout, timestamp)
			}

			//Upload CA root files to the nodes.
			for _, node := range d.Nodes {
				objects.WriteLog(d, fmt.Sprintf("Upload root CA files to node [%s]", node), stdout, timestamp)
		...
		for _, infra := range d.Infras {
			switch infra.Name {
			case InfraEtcd:
				objects.WriteLog(d, fmt.Sprintf("Deploy etcd in cluster"), stdout, timestamp)
				if err := infras.DeployEtcdInCluster(d, &infra, stdout, timestamp); err != nil {
					return err
				}
			case InfraFlannel:
				objects.WriteLog(d, fmt.Sprintf("Deploy flannel in cluster"), stdout, timestamp)
				if err := infras.DeployFlannelInCluster(d, &infra, stdout, timestamp); err != nil {
					return err
				}
			case InfraDocker:
				objects.WriteLog(d, fmt.Sprintf("Deploy docker in cluster"), stdout, timestamp)
				if err := infras.DeployDockerInCluster(d, &infra, stdout, timestamp); err != nil {
					return err
				}
			case InfraKubernetes:
				objects.WriteLog(d, fmt.Sprintf("Deploy k8s in cluster"), stdout, timestamp)
