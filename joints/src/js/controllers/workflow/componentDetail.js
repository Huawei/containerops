...
define(["app","services/diagram/main","services/component/main","services/component/io","services/component/check"], function(app) {
    app.controllerProvider.register('ComponentDetailController', ['$scope', '$stateParams', '$state', 'componentService', 'diagramService', 'componentIO',
        'componentCheck', 'notifyService', 'apiService', 'loading',
        function($scope, $stateParams, $state, componentService, diagramService, componentIO, componentCheck, notifyService, apiService, loading) {
            // tabs control
            ...
            $scope.saveComponent = function() {
                ...
                var workflowData = diagramService.workflowData;
                var currentStageIndex = diagramService.currentStageIndex;
                var currentActionIndex = diagramService.currentActionIndex;
                var currentComponentIndex = diagramService.currentComponentIndex;
                var id = workflowData[currentStageIndex]['actions'][currentActionIndex].id;

                var component = angular.copy($scope.component);
                    component.uuid = 'component-'+uuid.v1();

                workflowData[currentStageIndex]['actions'][currentActionIndex]['components'][currentComponentIndex] = component;

                diagramService.resetWorkflowData(workflowData);

                $state.go("workflow.create.action",{"id": id});
            ...
            function getComponentDetail(id) {
                ...
                $scope.component = workflowData[currentStageIndex]['actions'][currentActionIndex]['components'][currentComponentIndex];
            ...
            function init() {
                ...
                getComponentDetail($stateParams.id);
            }
            init();
        }
    ])
